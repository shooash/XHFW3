	...
	private static class AppIconButton {
		...
		public AppIconButton(Context context, int id_number, String package_name) {
			...
			try {
				...
				icon = new BitmapDrawable(getBitmap(ai.loadIcon(pm)));
			} catch (Exception e) {
				icon = new BitmapDrawable(getBitmap(context.getResources().getDrawable(android.R.drawable.sym_def_app_icon)));
			}
